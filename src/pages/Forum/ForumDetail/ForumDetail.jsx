    ...
    const fetchForumDetails = async () => {
        try {
          const [forumResponse, commentsResponse] = await Promise.all([
            axios.get(`http://localhost:8000/api/post/forum/${id}`, { headers: { Authorization: `Bearer ${GetToken()}` } }),
            axios.get(`http://localhost:8000/api/post/forum/${id}/comment`, { headers: { Authorization: `Bearer ${GetToken()}` } })
          ]);
            ...
            {comments.map((comment, index) => (
              <li key={index} className={cx('comment')}>
                ...
                <div className={cx('commentMeta')}>
                  <span className={cx('commentAuthor')}>{comment.author}</span>
